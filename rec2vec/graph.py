...
def records_to_graph():
    ...
    nodedict = {}
    # YOUR CODE HERE
    # A node is created for each entity: user, movie, director, genre, rating, actors
    nodeid = 0
    # Nodes for users
    for u_id in ratings:
        node = Node(id=nodeid, name=u_id, type="user")
        nodedict[u_id] = node
    ...
    # Nodes for movies
    for m_id in movies:
        node = Node(id=nodeid, name=m_id, type="movie")
        nodedict[m_id] = node
        nodelist.append(node)
        nodeid += 1
    ...
    for d_id in directors:
        node = Node(id=nodeid, name=d_id, type="director")
        nodedict[d_id] = node
        nodelist.append(node)
        nodeid += 1
    ...
    # Nodes for genres
    for g_id in genres:
        node = Node(id=nodeid, name=g_id, type="genre")
        nodedict[g_id] = node
        nodelist.append(node)
        nodeid += 1
    ...
    # Nodes for actors
    for a_id in actors:
        node = Node(id=nodeid, name=a_id, type="actor")
        nodedict[a_id] = node
        nodelist.append(node)
        nodeid += 1
    ...
    # Adding all edges...This will be done by modifying the neighbours list for each node.
    # YOUR CODE HERE

    # Add edges between users and movie-rating nodes
    for u_id in ratings:
        for m_id in ratings[u_id]:
            r_id = m_id + "_" + str(ratings[u_id][m_id])
            # Add neighbour Node r_id to user Node u_id and vice-versa
            nodedict[u_id].neighbors.append(nodedict[r_id])
            nodedict[r_id].neighbors.append(nodedict[u_id])
    ...
    for m_id in movies:
        ...
        for a_id in movies[m_id].actors:  # One movie, many actors
            # Add neighbour Node a_id to movie m_id and vice-versa
            nodedict[m_id].neighbors.append(nodedict[a_id])
            nodedict[a_id].neighbors.append(nodedict[m_id])

        for g_id in movies[m_id].actors:  # One movie, many genres
            # Add neighbour Node g_id to movie m_id and vice-versa
            nodedict[m_id].neighbors.append(nodedict[g_id])
            nodedict[g_id].neighbors.append(nodedict[m_id])
